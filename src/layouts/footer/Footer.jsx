import React from "react";

const Logo = () => (
  <svg
    width="161"
    height="33"
    viewBox="0 0 161 33"
    fill="none"
    xmlns="http://www.w3.org/2000/svg"
  >
    <path
      d="M28.8907 26.4478L26.5625 22.1194C26.0334 21.1406 25.0316 20.5243 23.938 20.5243H15.8105C15.4084 20.5243 15.0979 20.2053 15.0979 19.7921C15.0979 19.2265 15.5848 19.0815 15.8105 19.0598C19.9025 18.6828 23.2113 17.5735 22.8233 15.0141C22.4635 12.6142 18.3927 11.65 15.8035 11.4977C15.3449 11.4687 15.0909 11.0482 15.0909 10.7654C15.0909 10.4826 15.3167 10.0331 15.8599 10.0331L19.4651 9.99687C19.7684 9.99687 19.9589 9.65611 19.8108 9.38785L15.4295 1.18774H13.4541L10.4063 6.93C10.2581 7.20552 10.4557 7.53903 10.7591 7.53903H13.1719C13.5388 7.53903 13.8845 7.85078 13.8845 8.2713C13.8845 8.69181 13.5599 8.96008 13.1719 9.00358C9.67962 9.4241 7.67597 10.7799 7.52076 12.6795C7.35143 14.7168 9.82072 16.0074 13.1719 16.5657C13.5035 16.6237 13.8845 16.8339 13.8845 17.2979C13.8845 17.762 13.5105 18.0302 13.1931 18.0302L6.24378 18.0665C5.15729 18.0665 4.15546 18.6828 3.62633 19.6616L0 26.4261H12.5228L12.2477 32.7049H16.6289L16.3538 26.4406H28.8836L28.8907 26.4478Z"
      fill="#172B02"
    ></path>
    <path
      d="M51.2104 12.9194C51.2104 13.6662 50.6087 14.2752 49.8547 14.2752H42.8877C42.1409 14.2752 41.532 13.6662 41.532 12.9194V6.26361H37.5591V26.4412H41.532V18.9734C41.532 18.2266 42.1337 17.6176 42.8877 17.6176H49.8547C50.6015 17.6176 51.2104 18.2266 51.2104 18.9734V26.4412H55.1543V6.25635H51.2104V12.9121V12.9194Z"
      fill="#172B02"
    ></path>
    <path
      d="M64.3397 11.1572C60.0261 11.1572 56.481 14.1879 56.481 18.8788C56.481 23.5698 59.9681 26.7454 64.3397 26.7454C68.7114 26.7454 72.1768 23.6568 72.1768 18.8788C72.1768 14.1009 68.6896 11.1572 64.3397 11.1572ZM64.3397 23.6423C61.9183 23.6423 60.2798 21.7137 60.2798 18.8861C60.2798 16.0584 61.8966 14.2676 64.3397 14.2676C66.7829 14.2676 68.3706 16.1744 68.3706 18.8861C68.3706 21.7427 66.7322 23.6423 64.3397 23.6423Z"
      fill="#172B02"
    ></path>
    <path
      d="M137.78 11.1572C133.807 11.1572 130.153 14.1226 130.153 18.8571C130.153 23.5915 133.698 26.7599 137.925 26.7599C141.006 26.7599 144.203 25.3461 145.124 22.0617H141.492C140.796 23.3885 139.419 23.8453 138.034 23.8453C135.902 23.8453 134.17 22.3807 133.887 20.0099H145.262V19.2051C145.262 13.9051 141.804 11.1645 137.773 11.1645H137.78V11.1572ZM133.865 17.3852C134.126 15.2827 135.772 14.0356 137.729 14.0356C139.803 14.0356 141.296 15.3624 141.47 17.3852H133.865Z"
      fill="#172B02"
    ></path>
    <path
      d="M130.313 11.4767C130.052 11.4187 129.675 11.3896 129.276 11.3896C127.979 11.3896 127.145 11.6797 126.253 12.3684C125.767 12.7745 125.39 13.2602 125.071 13.7823V11.5057H121.468V26.4341H125.158V18.3935C125.158 15.4861 127.232 14.8988 128.849 14.8988C129.255 14.8988 129.595 14.8988 130.313 14.9568V11.4622V11.4767Z"
      fill="#172B02"
    ></path>
    <path
      d="M116.712 10.1496V7.87305H113.059V22.4317C113.059 25.7161 114.675 26.4629 117.205 26.4629C118.474 26.4629 119.112 26.4629 120.171 26.4049V23.5482H116.712V14.3258H120.171V11.5054H118.068C117.321 11.5054 116.712 10.8964 116.712 10.1496Z"
      fill="#172B02"
    ></path>
    <path
      d="M104.685 11.1572C100.712 11.1572 97.0581 14.1226 97.0581 18.8571C97.0581 23.5915 100.603 26.7599 104.83 26.7599C107.911 26.7599 111.108 25.3461 112.029 22.0617H108.397C107.701 23.3885 106.323 23.8453 104.939 23.8453C102.807 23.8453 101.074 22.3807 100.792 20.0099H112.167V19.2051C112.167 13.9051 108.709 11.1645 104.678 11.1645H104.685V11.1572ZM100.77 17.3852C101.031 15.2827 102.677 14.0356 104.634 14.0356C106.708 14.0356 108.201 15.3624 108.375 17.3852H100.77Z"
      fill="#172B02"
    ></path>
    <path
      d="M90.7798 11.1576C88.7063 11.1576 86.9156 12.0204 85.9152 13.6372C84.9655 11.9044 83.2038 11.1576 81.4493 11.1576C79.4919 11.1576 77.9621 11.9044 77.0704 13.2602V11.4984H73.4673V26.4268H77.1574V18.1179C77.1574 15.5803 78.6219 14.3188 80.2096 14.3188C81.7973 14.3188 83.008 15.3846 83.008 17.5452V26.4268H86.6982V18.1179C86.6982 15.6093 88.1626 14.3188 89.7503 14.3188C91.338 14.3188 92.5487 15.3846 92.5487 17.5452V26.4268H96.2679V16.9434C96.2679 12.731 93.7015 11.1504 90.7943 11.1504L90.7798 11.1576Z"
      fill="#172B02"
    ></path>
    <path
      d="M161.001 19.1978C161.001 13.8978 157.543 11.1572 153.512 11.1572C149.481 11.1572 145.885 14.1226 145.885 18.8571C145.885 23.5915 149.43 26.7599 153.657 26.7599C156.738 26.7599 159.935 25.3461 160.856 22.0617H157.224C156.528 23.3885 155.15 23.8453 153.766 23.8453C151.634 23.8453 149.902 22.3807 149.619 20.0099H160.994V19.2051H161.001V19.1978ZM149.597 17.378C149.858 15.2754 151.504 14.0283 153.461 14.0283C155.535 14.0283 157.028 15.3552 157.202 17.378H149.597Z"
      fill="#172B02"
    ></path>
  </svg>
);

const Section = ({ title, list = [] }) => {
  return (
    <div style={{ display: "grid", gap: "12px" }}>
      <div
        style={{
          fontFamily: "Roboto Condensed",
          fontSize: "24px",
          fontWeight: 700,
          lineHeight: "0.9",
          letterSpacing: "-0.04rem",
        }}
      >
        {title}
      </div>
      <div style={{ display: "grid", gap: "8px" }}>
        {list.map(({ alias, link }, index) => (
          <div key={index}>{alias}</div>
        ))}
      </div>
    </div>
  );
};

export default function Footer() {
  return (
    <div style={{ backgroundColor: "#ebebeb" }}>
      <div
        style={{
          display: "grid",
          gap: "60px",
          padding: "80px 40px",
          width: "85%",
          margin: "0 auto",
        }}
      >
        <Logo />
        <div
          style={{
            display: "grid",
            gridTemplateColumns: "repeat(3,1fr)",
            justifyContent: "space-between",
            alignItems: "start",
          }}
        >
          <Section
            title="Plans & Services"
            list={[
              { alias: "Accueil", link: "" },
              { alias: "Orientation", link: "" },
              { alias: "Smart Events", link: "" },
              { alias: "Smart News", link: "" },
              { alias: "Smart Club", link: "" },
              { alias: "Services", link: "" },
            ]}
          />
          <Section
            title="Étudier en Tunisie"
            list={[
              { alias: "Casablanca : +212 660 75 13 30", link: "" },
              { alias: "Libreville : +241 060 000 270", link: "" },
              { alias: "Abidjan : +225 0173 999 025", link: "" },
              { alias: "Kinshasa : +243 900 010 901", link: "" },
              { alias: "Brazzaville : +242056168573", link: "" },
              { alias: "Nouakchott : +0022247943838", link: "" },
            ]}
          />
          <Section
            title="Réseaux sociaux"
            list={[
              { alias: "Facebook", link: "" },
              { alias: "Instagram", link: "" },
              { alias: "Youtube", link: "" },
              { alias: "Linkedin", link: "" },
            ]}
          />
        </div>

        <div style={{ borderTop: "1px solid" }} />

        <div
          style={{
            fontSize: "14px",
            display: "flex",
            justifyContent: "space-between",
            alignItems: "center",
          }}
        >
          <p>
            Tout droits réservés <b>Smart Africa Student</b> ©
          </p>
          <p>Un site conçu et développé par BigFocus</p>
        </div>
      </div>
    </div>
  );
}
